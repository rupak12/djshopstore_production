    ...
    @if (count($featured_categories) > 0)
        <section class="mb-4 mt-4">
            <div class="container">
                ...
                <!-- Categories Grid -->
                <div class="row row-cols-xl-6 row-cols-lg-4 row-cols-md-3 row-cols-2 gutters-10 gutters-md-16">
                    @foreach ($featured_categories->take(6) as $category)
                        <div class="col mb-3">
                            <a href="{{ route('products.category', $category->slug) }}" 
                            class="d-block text-reset h-100 bg-white overflow-hidden category-card">
                                <!-- Category Image -->
                                <div class="position-relative text-center h-180px">
                                    <img src="{{ isset($category->bannerImage->file_name) 
                                        ? my_asset($category->bannerImage->file_name) 
                                        : static_asset('assets/img/placeholder.jpg') }}"
                                        alt="{{ $category->getTranslation('name') }}"
                                        class="lazyload h-100 img-fit has-transition"
                                        onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}'">
                                </div>

                                <!-- Category Details -->
                                <div class="px-3 pt-3 pb-3 text-center">
                                    <h3 class="fw-600 fs-14 text-truncate">{{ $category->getTranslation('name') }}</h3>
                                </div>
                            </a>
                        </div>
                    @endforeach
